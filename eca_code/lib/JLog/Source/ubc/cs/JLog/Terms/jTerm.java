...
abstract public class jTerm extends jType implements iTerm
{
  ...
  *
  * @param term		the <code>jTerm</code> to compare with this instance of 
  *			<code>jTerm</code>.
  * @param var_equal	if <code>var_equal</code> is <code>true</code>, then unbound 
  *			variables are considered equal.
  *                     <code>var_equal</code> = <code>false</code> is standard prolog 
  * 			behavior.
  * @return 		<code>LESS_THAN</code> if this instance is less than 
  * 			<code>term</code>, 
  * 			<code>EQUAL</code> if the two terms are equal, and 
  * 			<code>GREATER_THAN</code> if 
  * 			this instance is greater than <code>term</code>.
  */
  ...
  * provided <code>jTerm</code>. sub-classes must override.    
  *
  * @param term		the <code>jTerm</code> to compare with this instance of 
  * 			<code>jTerm</code>. if 
  * 			<code>term</code> is an unknown type then, if 
  * 			<code>first_call</code> is 
  * 			<code>true</code>, call term.compare(this,false) and return 
  * 			the negative result, otherwise return <code>EQUAL</code>.
  * @param first_call	<code>true</code> if just invoked by public 
  * 			<code>compare</code> member function.
  ...
  * to unbind all variables. Terms such as <code>cut</code>, which jump back to the 
  ...
  * of this term) to the <code>jUnifiedVector</code>
  *
  * @param v 		The <code>jUnifiedVector</code> where unbound variables are
  ...
  *
  * @param v 		The <code>jUnifiedVector</code> where variables are
  ...
  * of this term) to the <code>jVariableVector</code>
  *
  * @param v 		The <code>jVariabeVector</code> where variables are
  * 			added to. This parameter is used to as output to the 
  *			caller, not as input.
  * @param all 		If <code>true</code>, then all variables should register,
  ...
  * call only for terms for which <code>registerVariables</code> has already been 
  * invoked.
  * Within <code>duplicate</code>, any other calls to duplicate should pass along 
  * the same <code>vars</code> array produced by <code>registerVariables</code>
  * since the same duplication path previously taken by <code>registerVariables</code>
  * should be taken by <code>duplicate</code>. This call is designed only for terms 
  ...
  *			efficient for <code>jVariables</code> to return their
  * 			unique duplicate.
  ...
  * <code>duplicate</code> function. 
  * Sub-classes need not override, only implement the abstract 
  * <code>copy(vars)</code> member function. copy is not as efficient as 
  * <code>duplicate</code>.
  *
  * @return 		<code>jTerm</code> which is an instantiated copy of this
  *			term.
  */
 public jTerm 		copy()
  ...
  * <code>duplicate</code> function.  Should only be invoked by <code>copy()</code>
  * or other <code>copy(vars)</code>. Should only invoke other 
  * <code>copy(vars)</code> functions.
  ...
  * 			same <code>vars</code> ensures that the same variable 
  ...
  * 			<code>false</code> is the default for displaying the term, 
